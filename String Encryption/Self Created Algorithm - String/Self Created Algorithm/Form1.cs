...
namespace Self_Created_Algorithm
{
    public partial class Form1 : Form
    {
        ...
        private static string Swap(String str) // Method that completes Step 1: Swap the first letter with the last letter in the string and store it in newString.   || eg. James becomes sameJ
        {
            //Variables
        ...
        private static string DecrytedStr(string str)
        {
            ...
            for (int i = 0; i < str.Length; i++) //For loop for moving characters 3 spaces down on the ASCII Table and turning array into one string.  
            {
                CharInArray = arr[i];
        ...
        private void btnEncrypt_Click(object sender, EventArgs e)
        {
            ...
            {
                str = txtString.Text;
                lblEncrypt.Text = EncrytedStr(ReverseString(Swap(str)));
        ...
        private void btnDecrypt_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                string x, y, EncryptedString, DecryptedString = "";

                EncryptedString = EncrytedStr(ReverseString(Swap(str)));
                x = DecrytedStr(EncryptedString);
                y = ReverseString(x);
                DecryptedString = Swap(y);

                lblDecrypt.Text = DecryptedString;
